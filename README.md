...
Normally an agent stops to ask: *which approach? can I write this file? is this okay?* Fine when you're sitting there — useless when you've stepped away. **Coffee Time flips the default: when the agent would ask, it decides instead.** It picks a sensible, reversible default, logs the decision, and keeps moving. It only freezes for the genuinely dangerous stuff (force-pushing main, DB migrations, spending money, sending email, deploying to prod).
...
Without that, the agent still freezes on the first file write asking permission. Brain (skill) + hands (acceptEdits) + legs (its task list) — you flip the one switch, it does the rest.
